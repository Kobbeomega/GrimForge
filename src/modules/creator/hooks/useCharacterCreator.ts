...
export function useCharacterCreator({
  ...
  const [state, setState] = useState<CharacterCreatorState>(() => ({
    draft:
      initialDraft ??
  ...
  const currentStep = useMemo(
    () =>
      creatorSteps.find(
        (step) => step.id === state.draft.currentStep,
      ) ?? creatorSteps[0],
  ...
  function setCurrentStep(stepId: CreatorStepId) {
    updateDraft({
      currentStep: stepId,
    });
  }

  function markStepCompleted(stepId: CreatorStepId) {
    setState((currentState) => {
  ...
  function markStepIncomplete(stepId: CreatorStepId) {
    setState((currentState) => ({
      ...currentState,
      completedSteps:
  ...
  function goToNextStep() {
    ...
    markStepCompleted(state.draft.currentStep);
    setCurrentStep(nextStep);
  }

  function goToPreviousStep() {
    const previousStep = getPreviousCreatorStep(
      state.draft.currentStep,
    );
  ...
  function resetCreator() {
    setState({
      draft: createEmptyCharacterDraft(fileNumber),
      completedSteps: [],
    });
  }
